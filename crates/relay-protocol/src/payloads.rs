...
pub struct DockerInspectResponse {
    pub inspect_json: serde_json::Value,
}
...
// Not in current spec — compose.down covers stop
// #[derive(Debug, Clone, Serialize, Deserialize)]
...
mod tests {
    ...
    fn register_request_roundtrip() {
        ...
        assert_eq!(parsed.relay_version, "0.1.0");
    }
    ...
    fn register_response_roundtrip() {
        ...
        assert_eq!(parsed.session_key, "key-abc");
    ...
    #[test]
    fn docker_inspect_request_roundtrip() {
        let req = DockerInspectRequest {
            container: "nginx".into(),
        };
        let json = serde_json::to_string(&req).unwrap();
        let parsed: DockerInspectRequest = serde_json::from_str(&json).unwrap();
        assert_eq!(parsed.container, "nginx");
    }

    #[test]
    fn docker_inspect_response_roundtrip() {
        let resp = DockerInspectResponse {
            inspect_json: serde_json::json!({
                "Id": "abc123",
                "Name": "/nginx",
                "State": {"Running": true}
            }),
        ...
        let json = serde_json::to_string(&resp).unwrap();
        let parsed: DockerInspectResponse = serde_json::from_str(&json).unwrap();
        assert_eq!(parsed.inspect_json["Id"], "abc123");
        assert_eq!(parsed.inspect_json["Name"], "/nginx");
        assert!(parsed.inspect_json["State"]["Running"].as_bool().unwrap());
    }
    ...
    fn fs_entry_roundtrip() {
        ...
        assert_eq!(parsed.name, "compose.yml");
    ...
    fn fs_list_response_roundtrip() {
        ...
        assert_eq!(parsed.entries.len(), 2);
        assert_eq!(parsed.entries[0].name, "compose.yml");
