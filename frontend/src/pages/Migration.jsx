...
export default function Migration({ navigate }) {
  ...
  const handleStrategyUpdate = useCallback((s) => {
    setStrategy(s);
    if (s.transfer_method) setTransferMethod(s.transfer_method);
  ...
  const allCriticalResolved = () => {
    ...
    return conns.filter(c => c.will_break).every(c => connectionResolutions[c.var_name]?.resolved);
  };
  ...
  const renderStepContent = () => {
    switch (step) {
      ...
      case 2:
        ...
        const dbConns = analysis.db_connections || [];
        return (
          <div style={{ display: 'grid', gap: '16px' }}>
            {/* Container info */}
            <div className="card">
              ...
              <table>
                <tbody>
                  <tr><td style={{ color: 'var(--text-secondary)', width: '120px' }}>Name</td><td className="mono" style={{ fontWeight: 500 }}>{analysis.container_name || '—'}</td></tr>
                  <tr><td style={{ color: 'var(--text-secondary)' }}>Image</td><td className="mono">{analysis.image || '—'}</td></tr>
                  <tr><td style={{ color: 'var(--text-secondary)' }}>ID</td><td className="mono" style={{ fontSize: '0.8rem' }}>{analysis.container_id || '—'}</td></tr>
                </tbody>
              </table>
            </div>
            ...
            {volumes.length > 0 && (
              <div className="card">
                ...
                <table>
                  ...
                  <tbody>
                    {volumes.map(v => {
                      const sizeGB = v.size_bytes ? (v.size_bytes / 1073741824).toFixed(1) : '—';
                      return (
                        <tr key={v.name}>
                          <td className="mono" style={{ fontWeight: 500 }}>{v.name}</td>
                          <td>{v.driver || '—'}</td>
                          <td>{v.driver_category || '—'}</td>
                          <td className="mono">{sizeGB === '—' ? '—' : `${sizeGB} GB`}</td>
                          <td>{v.shared ? '🔗 Yes' : 'No'}</td>
                          <td><span className="mono" style={{ fontSize: '0.75rem', color: 'var(--accent)' }}>{v.transfer_method || '—'}</span></td>
                          <td>
              ...
              <div className="card">
                ...
                <ConnectionReview
                  connections={dbConns.map(c => ({
                    ...c,
                    resolved: connectionResolutions[c.var_name]?.resolved || false,
                    resolution: connectionResolutions[c.var_name]?.action,
                  }))}
            ...
            {analysis.estimated_size_bytes > 0 && (
              <div className="card" style={{ borderLeft: '3px solid var(--accent)' }}>
                ...
                <div className="mono" style={{ fontSize: '1.2rem', color: 'var(--accent)' }}>
                  {(analysis.estimated_size_bytes / 1073741824).toFixed(1)} GB
                      ...
                      return (
                        <tr key={i}>
                          ...
                          <td className="mono" style={{ fontSize: '0.8rem' }}>
                            {isRevealed ? (env.value_masked || env.value || '') : (
            ...
            {!dryRunResult ? (
              ...
              <>
                ...
                <div className="card">
                  ...
                  <table>
                    <tbody>
                      <tr><td style={{ color: 'var(--text-secondary)', width: '180px' }}>Migration ID</td><td className="mono">{dryRunResult.migration_id || '—'}</td></tr>
                      <tr><td style={{ color: 'var(--text-secondary)' }}>Source</td><td>{dryRunResult.source_endpoint || sourceEndpoint}</td></tr>
                      <tr><td style={{ color: 'var(--text-secondary)' }}>Target</td><td>{dryRunResult.target_endpoint || targetEndpoint}</td></tr>
                      <tr><td style={{ color: 'var(--text-secondary)' }}>Container</td><td className="mono">{dryRunResult.container_name || '—'}</td></tr>
                      <tr><td style={{ color: 'var(--text-secondary)' }}>Estimated Size</td><td className="mono">{dryRunResult.estimated_size_bytes > 0 ? `${(dryRunResult.estimated_size_bytes / 1073741824).toFixed(1)} GB` : '—'}</td></tr>
