...
export default function MigrationPlan({ plan = {}, volumes = [], onUpdate }) {
  const [transferMethod, setTransferMethod] = useState(plan.transfer_method || 'rsync-over-ssh');
    ...
    <div style={{ display: 'grid', gap: '20px' }}>
                ...
                return (
                  <tr key={v.name}>
                    ...
                    <td className="mono" style={{ fontSize: '0.8rem' }}>
                      {v.size_bytes ? `${(v.size_bytes / 1073741824).toFixed(1)} GB` : '—'}
                    </td>
                    <td>
                      <select
                        value={override.transfer_method || v.transfer_method || transferMethod}
                        onChange={(e) => handleVolumeOverride(v.name, 'transfer_method', e.target.value)}
                        style={{ fontSize: '0.75rem', padding: '4px 8px' }}
                      >
                        <option value="inherit">Inherit ({v.transfer_method || transferMethod})</option>
                        {TRANSFER_METHODS.map(m => (
      ...
      <div className="card" style={{ borderLeft: '3px solid var(--accent)' }}>
        ...
        <div style={{ display: 'grid', gap: '6px', fontSize: '0.85rem' }}>
          ...
          {plan.estimated_size_bytes > 0 && (
            <div>
              <span style={{ color: 'var(--text-secondary)' }}>Estimated size: </span>
              <span className="mono">
                {(plan.estimated_size_bytes / 1073741824).toFixed(1)} GB
